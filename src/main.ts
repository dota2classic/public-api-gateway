...
async function test(app: INestApplication<any>){
  ...
  const ids = r.teams.flatMap(t => t.parties).flatMap(t => t.players).map(it => it.playerId);


  const qbus = await app.get(QueryBus);
  // @ts-ignore
  const results: GetPlayerInfoQueryResult[] = await Promise.all(ids.map(it => qbus.execute(new GetPlayerInfoQuery(it, Dota2Version.Dota_684))));
  // @ts-ignore
  const results2: GetUserInfoQueryResult[] = await Promise.all(ids.map(it => qbus.execute(new GetUserInfoQuery(it))));;

  console.log(
    JSON.stringify(results.map(it => ({
      ...it,
