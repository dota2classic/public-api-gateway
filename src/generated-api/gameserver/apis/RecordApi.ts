...
import {
  ...
  GameserverPlayerYearSummaryDtoFromJSON,
} from "../models";
...
export interface RecordControllerYearResultsRequest {
    steamId: string;
}
...
export class RecordApi extends runtime.BaseAPI {
    ...
    /**
     */
    recordControllerYearResultsContext(requestParameters: RecordControllerYearResultsRequest): runtime.RequestOpts {
        const queryParameters: any = {};

        const headerParameters: runtime.HTTPHeaders = {};

        return {
            path: `/record/year/{steamId}`.replace(`{${"steamId"}}`, encodeURIComponent(String(requestParameters.steamId))),
            method: 'GET',
            headers: headerParameters,
            query: queryParameters,
        };
    ...
    /**
     */
    recordControllerYearResults = async (steamId: string): Promise<GameserverPlayerYearSummaryDto> => {
        const response = await this.recordControllerYearResultsRaw({ steamId: steamId });
        return await response.value();
    }

    /**
     */
    private async recordControllerYearResultsRaw(requestParameters: RecordControllerYearResultsRequest): Promise<runtime.ApiResponse<GameserverPlayerYearSummaryDto>> {
        this.recordControllerYearResultsValidation(requestParameters);
        const context = this.recordControllerYearResultsContext(requestParameters);
        const response = await this.request(context);

        return new runtime.JSONApiResponse(response, (jsonValue) => GameserverPlayerYearSummaryDtoFromJSON(jsonValue));
    }

    /**
     */
    private recordControllerYearResultsValidation(requestParameters: RecordControllerYearResultsRequest) {
        if (requestParameters.steamId === null || requestParameters.steamId === undefined) {
            throw new runtime.RequiredError('steamId','Required parameter requestParameters.steamId was null or undefined when calling recordControllerYearResults.');
        }
    }
