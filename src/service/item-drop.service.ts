  ...
  public async onItemDrop(evt: ItemDroppedEvent) {
    ...
    try {
      ...
      await this.forumApi.forumControllerPostMessage(
        ...
        {
          content: `Игроку https://dotaclassic.ru/players/${evt.steamId} выпала награда <${evt.item.rarity}>"${evt.item.marketHashName}"</${evt.item.rarity}> ${evt.matchId ? `за игру в матче https://dotaclassic.ru/matches/${evt.matchId}` : ``}\n https://community.fastly.steamstatic.com/economy/image/${evt.item.icon}/item.png`,
          author: {
            steam_id: "159907143",
            roles: [],
          },
